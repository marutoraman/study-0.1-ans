...
# 以下でも同様の結果だが、文字列の+での連結は、可読性が低いため非推奨
...
def print_charactors_name():
    for name in names:
        print(name)

print_charactors_name()
...
names=["たんじろう","ぎゆう","ねずこ","むざん"]
def exists_charactor(name):
    '''
    関数名は、動詞＋名詞とし、何をする関数なのかを明確に表すのがベスト
    本関数では、exists(存在確認) + charactor（キャラクター）としている。
    '''
    if name in names:
        print(f"{name}は、namesに含まれます。")
    else:
        print(f"{name}は、namesに含まれません。")
        
exists_charactor("ぎゆう")
